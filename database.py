...
def toggle_habit_status_from_db(position: int, new_status:int):
    with conn:
        c.execute('UPDATE habits SET status = :status, last_date_checked = :last_date_checked WHERE position = :position ',
                  {'position': position, 'status': new_status, 'last_date_checked': datetime.date.today()}) #status 2 =completed, get date when completed
...
def get_habit_by_position(position: int):
    ...
    while row is not None:
        ...
        with conn:
            c.execute('SELECT position, streak, last_date_checked FROM habits WHERE position = :position', {'position': position}) #select all from todos
            return {'position': c.fetchone()[0], 'streak': c.fetchone()[1], 'last_date_checked': c.fetchone()[2]}
